...
def alter_max(A, Hd, P_max, sigma2):
...
def maxmin_perfect_csi(params_system, Rician_factor,sigma2_input):
    ...
    Hd = channels[0]
    theta_all, g, p, rate_min_all, rate_all, w_all = alter_max(A, Hd,P_max=1, sigma2=sigma2_input)

    rate_nn, rate_nn_all = compute_rate_batch(w_all.conjugate(), theta_all, channels,sigma2=sigma2_input)
    rate_min, rate_min_all = compute_minrate_batch(w_all.conjugate(), theta_all, channels,sigma2=sigma2_input)

    file_path = './channel_data/perfect_csi' + str(params_system) + '_' + str(Rician_factor)+'_'+str(sigma2_input) + '.mat'
...
def maxmin_imperfect_csi(params_system, len_pilot, Rician_factor, noise_power_db, sigma2_input):
    ...
    rate_nn, rate_nn_all = compute_rate_batch(w_all.conjugate(), theta_all, channel_true,sigma2= sigma2_input)
    rate_min, rate_min_all = compute_minrate_batch(w_all.conjugate(), theta_all, channel_true,sigma2= sigma2_input)

    file_path = './channel_data/imperfect_csi' + str(params_system) + '_' + str(Rician_factor)  + '_' + str(len_pilot) + '_' + str(noise_power_db)+'_'+str(sigma2_input) + '.mat'
    sio.savemat(file_path,
                {'w_all': w_all,
                 'theta_all': theta_all,
                 'rate_min' : rate_min,
...
def main_imperfect_csi(params_system,Rician_factor,noise_power_db,sigma2_input):
    num_user = params_system[2]
    set_len_pilot = np.array([5,25])*num_user
    set_rate_min,set_rate_all = [],[]
    for len_pilot in set_len_pilot:
        print('len_pilot',len_pilot)
        rate_min, rate_all = maxmin_imperfect_csi(params_system, len_pilot, Rician_factor, noise_power_db,sigma2_input)
        set_rate_min.append(rate_min)
        set_rate_all.append(rate_all)

        filename = 'maxmin_bl'+ str(params_system) + '_' + str(Rician_factor) + '_'  + str(noise_power_db)+'_'+str(sigma2_input)+'.mat'
