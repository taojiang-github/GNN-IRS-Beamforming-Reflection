...
from plot_bf_pattern.generate_received_pilots import  generate_pilots_bl, generate_pilots_bl_v2, \
...
def main_rate_vs_pilot(len_pilots_tmp, params_system, input_flag, Rician_factor, noise_power_db, Pt_u, Pt_d):
    ...
    (num_antenna_bs, num_elements_irs, num_user) = params_system
    location_user = None
    learning_rate, max_epochs, batch_size, iter_per_epoch = 1e-3, 100, 1024, 100
    ...
    for len_pilot in set_len_pilot:
        ...
        sio.savemat('results_train_sumrate_elevation' + str(input_flag) + '.mat',
...
if __name__ == '__main__':
    ts = time.time()
    main_rate_vs_pilot(len_pilots_tmp=np.array([25]),
                       input_flag=0,
                       params_system=(8, 200, 1),
                       Rician_factor=10,
                       noise_power_db=-100,
                       Pt_u = 15,
                       Pt_d = 5)
